...
import math
import torch
from ocr_model.modules.deform_ops import ModulatedDeformConv2d
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=None):
        ...
        deformable_groups = dcn.get("deformable_groups", 1) if dcn else 1
        # offset_channels = 27
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=Optional[dict]):
        ...
        self.conv2 = ModulatedDeformConv2d(
            ...
            padding=1,
            stride=stride,
            groups=dcn.get("deformable_groups", 1) if dcn is not None else 1,
            bias=False,
        )
        self.bn2 = BatchNorm2d(planes)
        ...
        if self.dcn is not None:
            for m in self.modules():
                if isinstance(m, Bottleneck) or isinstance(m, BasicBlock):
                    if hasattr(m.conv2, "offset_conv_mask"):

                        constant_init(m.conv2.offset_conv_mask, 0)
